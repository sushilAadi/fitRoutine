 ...
 *                           - effectiveCompletionPercent: ((Completed + Skipped - Deleted) / Total Planned) * 100
 */
export const calculateDetailedWorkoutProgress = (transformData, firebaseStoreData) => {
  try {
    ...
    // New metric: Effective completion including completed + skipped - deleted
    const totalEffectiveCompletions = totalCompletedPlannedSets +  totalSkippedPlannedSets  + totalDeletedSets;
    const effectiveCompletionPercent = calculatePercentageUnclamped(totalEffectiveCompletions, totalPlannedSets);
    ...
    return {
      ...
      effectiveCompletionPercent,       // NEW: % of (completed + skipped - deleted) sets relative to planned sets
      totalNewSets,
      totalDeletedSets,
    };
