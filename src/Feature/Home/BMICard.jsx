...
const BMICard = ({ bmi, lastUpdate = '9 Jan, 2024 at 10:12' }) => {
  ...
  const segments = [
    { start: 0, end: 18.5, color: 'bg-gradient-to-r from-cyan-800 to-green-600' },
    { start: 18.5, end: 25, color: 'bg-gradient-to-r from-green-600 to-green-800' },
    { start: 25, end: 30, color: 'bg-gradient-to-r from-amber-400 to-orange-600' },
    { start: 30, end: maxBMI, color: 'bg-gradient-to-r from-orange-600 to-red-600' }
  ]
  ...
  const getBMIStatus = () => {
    ...
    if (bmi >= 25 && bmi < 40) return "Your BMI indicates you're overweight. Consider adopting a healthier lifestyle with balanced diet and regular exercise."
    ...
    <div className="px-4 mx-auto mt-4 bg-white ">
      ...
      <div className="space-y-4">
        ...
        {segments.map(({ start, end, color }, i) => (
          <div
            key={i}
            className={`absolute top-0 h-full ${color} first:rounded-l-lg  ${end ===40  && 'rounded-r-lg'}`}
            style={{
        ...
        <div
          className={`absolute top-1/2 w-8 h-8 -mt-4 -ml-4 flex items-center justify-center rounded-full ${getColor(bmi)} text-white shadow-md transition-all duration-300`}
